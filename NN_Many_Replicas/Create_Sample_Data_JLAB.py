...
from JLAB_Deuteron_Lineshape import FitFunc
g = 0.05
s = 0.04
bigy=(3-s)**0.5
labelfontsize = 30
...
for x in range(0,1):
    ...
    result = FitFunc(w,0.069,1,1.801,0.027,32.682,-0.02,-0.0)
    normalizedData = (result-np.min(result))/(np.max(result)-np.min(result)) 
