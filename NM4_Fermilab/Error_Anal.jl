...
function GenerateLineshape(P, x)
    ...
    function cosaltwo(x, eps)
        arg = (1 .+ cosal(x, eps)) ./ 2
        return sqrt.(arg)  # Allow complex results
    end
...
data_min = -3.0  # Minimum value of the data range
data_max = 3.0  # Maximum value of the data range
polarization_values = vcat(0.005:0.05:.8)  # Polarization range from 0 to 1 and back

# Generate continuous data and bin it for each polarization value
x_values = range(data_min, stop=data_max, length=n)  # Generate n points in the range [-3, 3]
...
# Animation 1: 3D Surface Plot + 1D Signal Plot
println("Creating 3D Surface + 1D Signal Animation...")

# Create a sequence of indices for forward and backward motion
...
anim1 = @animate for idx in animation_indices
    ...
    # 3D Surface Plot (up to the current polarization value, in original order)
    ...
    # Combine plots with adjusted margins and layout
...
# Animation 2: 2D Heatmap + 1D Signal Plot
...
anim2 = @animate for idx in animation_indices
    ...
    # 2D Heatmap (up to the current polarization value, in original order)
