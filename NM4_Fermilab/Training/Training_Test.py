...
# When creating the model, replace the Lambda layer with a custom layer
...
# Custom callback to track metrics specifically in the 0-0.1 range
class RangeSpecificMetricsCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # Filter for values in 0-0.1 range
        mask = y_val <= 0.1
        if np.sum(mask) > 0:  # Check if we have values in this range
...
# Modify data preparation to ensure good representation around 0.05%
print("Loading and preparing data...")
data = pd.read_csv(data_path)

# Enhanced oversampling for critical range
...
if len(critical_range_data) > 0:
    # Oversample critical range more aggressively
...
history = model.fit(
    ...
    validation_data=(X_val, y_val),
    epochs=1000,
    batch_size=128,  # Reduced batch size for better precision
    callbacks=callbacks_list,
    verbose=2
)

# When loading the model, enable unsafe deserialization
tf.keras.config.enable_unsafe_deserialization()
model = tf.keras.models.load_model(os.path.join(model_dir, 'best_model.keras'), compile=False)
# Load the model with the training history
history_file = os.path.join(performance_dir, 'training_log.csv')
history = pd.read_csv(history_file)


# Recompile with the custom loss
optimizer = optimizers.AdamW(
