...
def residual_block(x, units, l2_reg=0.01, dropout_rate=0.2, Momentum=0.99, Epsilon=1e-5):
...
def Polarization_Model(params):
    ...
    x = layers.BatchNormalization(momentum=Momentum, epsilon=Epsilon)(inputs)
    x = layers.Dense(initial_units, activation='swish', 
                    kernel_initializer=initializers.HeNormal(),
                    kernel_regularizer=regularizers.l2(l2_reg_1))(x)
    x = layers.Dropout(input_dropout_rate)(x)
    
    
    for i in range(num_layers):
        units = units_per_layer[i]
        x = residual_block(x, units, l2_reg, dropout_rate, Momentum, Epsilon)
        
    x = layers.Dense(16, activation='swish', 
                    kernel_initializer=initializers.HeNormal(),
                    kernel_regularizer=regularizers.l2(l2_reg_1))(x)
