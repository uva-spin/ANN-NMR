...
version = 'Deuteron_Low_ResNet_Optuna_V3'  
...
def residual_block(x, units, l2_reg=0.01, dropout_rate=0.2, noise_stddev=0.1, Momentum=0.99, Epsilon=1e-5):

    y = layers.GaussianNoise(stddev=noise_stddev)(x)
    ...
    y = layers.GaussianNoise(stddev=noise_stddev)(y)
...
def Polarization_Model(params):
    ...
    # input_dropout_rate = params['input_dropout_rate']
    # initial_units = params['initial_units']
    # l1_reg_1 = params['l1_reg_1']
    # l2_reg_1 = params['l2_reg_1']
    # Momentum = params['Momentum']
    # Epsilon = params['Epsilon']
    # input_noise_stddev = params['input_noise_stddev']
    # hidden_noise_stddev = params['hidden_noise_stddev']
    # output_noise_stddev = params['output_noise_stddev']
    
    x = layers.GaussianNoise(stddev=params['input_noise_stddev'])(inputs)
    
    x = layers.LayerNormalization(momentum=params['Momentum'], epsilon=params['Epsilon'])(x)
    x = layers.Dense(params['initial_units'], activation='swish', 
                    kernel_initializer=initializers.HeNormal(),
                    kernel_regularizer=regularizers.l1(params['l1_reg_1']),
                    kernel_regularizer=regularizers.l2(params['l2_reg_1']))(x)
    x = layers.Dropout(params['input_dropout_rate'])(x)
    
    x = layers.GaussianNoise(stddev=params['hidden_noise_stddev'])(x)
    
    for i in range(params['num_layers']):
        units = params['units_per_layer'][i]
        x = residual_block(x, units, params['l2_reg'], params['dropout_rate'], 
                          params['hidden_noise_stddev'], params['Momentum'], params['Epsilon'])
    
    x = layers.GaussianNoise(stddev=params['output_noise_stddev'])(x)
    
    x = layers.Dense(params['final_units'], activation='swish', 
                    kernel_initializer=initializers.HeNormal(),
                    kernel_regularizer=regularizers.l2(params['l2_reg_1']))(x)
    ...
    loss_function = HighPrecisionLoss(alpha=params['Alpha'], beta=params['Beta'], gamma=params['Gamma'])
...
def objective(trial):
    ...
    params = {
        ...
        'input_dropout_rate': trial.suggest_float('input_dropout_rate', 0.0, 0.5),
        'dropout_rate': trial.suggest_float('dropout_rate', 0.0, 0.5),
        'initial_units': trial.suggest_int('initial_units', 256, 1024),
        'final_units': trial.suggest_int('final_units', 1, 512),
...
if __name__ == "__main__":
    try:
        ...
        best_params = {
            ...
            'input_dropout_rate': trial.params['input_dropout_rate'],
