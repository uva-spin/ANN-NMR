...
version = 'Deuteron_Low_Autoencoder_V2_LogCosh'  
performance_dir = f"Model Performance/{version}"  
model_dir = f"Models/{version}"  
os.makedirs(performance_dir, exist_ok=True)
os.makedirs(model_dir, exist_ok=True)

def Autoencoder(input_shape=(500,), encoding_dim=5000):
    # Encoder
    inputs = layers.Input(shape=input_shape, dtype='float32')
    x = layers.LayerNormalization()(inputs)
    
    # Encoding layers
    x = layers.Dense(2500, activation='swish', kernel_initializer='he_normal')(x)
    x = layers.Dropout(0.2)(x)
    encoded = layers.Dense(encoding_dim, activation='swish', kernel_initializer='he_normal')(x)

    # Decoder
    x = layers.Dense(2500, activation='swish', kernel_initializer='he_normal')(encoded)
    x = layers.Dropout(0.2)(x)
    decoded = layers.Dense(1, activation='sigmoid', kernel_initializer='he_normal')(x)  # Final output of dimension 1
